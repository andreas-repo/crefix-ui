...
@Component({
  selector: 'app-measurement-three-image-frame',
  imports: [
    CameraComponent
  ],
  templateUrl: './measurement-three-image-frame.component.html',
  styleUrl: './measurement-three-image-frame.component.css'
})
export class MeasurementThreeImageFrameComponent {
  ...
  constructor(private router: Router, private route: ActivatedRoute, dataService: DataService) {
    ...
    firstValueFrom(this.dataService.getCmDocumentById(this.id)).then((data: ICmDocumentJson) => {
      ...
      this.measurement_id = (<CmDocumentJson> data).measurementThreeId;
  ...
  returnOnePage() {
    this.router.navigate(['/measurement-three-frame', this.measurement_id]);
  }
  ...
  async saveCapturedImage() {
    ...
    await firstValueFrom(this.dataService.getMeasurementById(this.measurement_id)).then((data: IMeasurementJson) => {
      ...
      console.log("Received data from /getMeasurementById for MEASUREMENT THREE: " + JSON.stringify(data));
    });
