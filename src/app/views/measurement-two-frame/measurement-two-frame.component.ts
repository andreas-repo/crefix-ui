...
export class MeasurementTwoFrameComponent {
  tab_label : string = 'Zweiter Messpunkt';
  ...
  async saveMeasurementTwoData() {
    ...
    await this.router.navigate(['/measurement-two-image-frame', this.id]);
  }

  returnOnePage() {
    this.router.navigate(['/measurement-one-image-frame', this.id]);
  }
}
